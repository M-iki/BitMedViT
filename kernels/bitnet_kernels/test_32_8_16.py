...
def bitnet_int8xint2_linear(input0, input1, s, ws, ret):
    ...
    ret = ret.reshape(M_blocks, N // N_per_loop, M // M_blocks, N_per_loop)
    ret = ret.permute(0, 2, 1, 3)
    ret = ret.reshape(1, M, N).contiguous()
...
if __name__ == '__main__':
    ...
    for N,K in test_list:
        ...
        weight_compressed = weight_compressed.reshape([N // 16, K // 16 // 2, 4, 8, 4])
        # dat = dat.reshape([N // 16, K // 16, 4, 8, 4]).contiguous()

        n_cols = weight_compressed[:, :, [0, 1]]
        k_cols = weight_compressed[:, :, [2, 3]]

        out = torch.stack([n_cols, k_cols], dim = 1).contiguous()
        weight_compressed = out.view(N, K // 4).contiguous()
        ...
        for i in range(1):
            ...
            ret = torch.zeros((1, M, N), dtype=torch.bfloat16, device=input0.device)
        ...
        t1 = benchmark.Timer(
            stmt="torch.matmul(input0.to(torch.bfloat16),weight_bf16.T) / s / ws",
            setup="from __main__ import input0, weight_bf16, s, ws",
            num_threads=1,
        )
