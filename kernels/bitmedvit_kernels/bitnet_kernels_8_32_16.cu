#include "bitnet_kernels_8_32_16.h"
...
extern "C" void bitlinear_int8xint2(int8_t* input0, int8_t* input1, __nv_bfloat16* output0, __nv_bfloat16* s, __nv_bfloat16* ws, int M, int N, int K, cudaStream_t stream){
    if(M == 1024 && N == 512 && K == 2048){
        ladder_int8xint2_kernel<1024, 512, 2048, 4, 32, 8, 4, 4><<<dim3(4, 32, 1), dim3(8, 4, 4), 0, stream>>>(input0, input1, output0, s, ws);
    }
    else if(M == 1024 && N == 2048 && K == 512){
        ladder_int8xint2_kernel<1024, 2048, 512, 8, 32, 8, 4, 8><<<dim3(8, 32, 1), dim3(8, 4, 8), 0, stream>>>(input0, input1, output0, s, ws);
    } 
